/* ════════════════════════════════════════════════════════════
   AUTHENTIFICATION MICROSOFT (MSAL.js — contrôle d'accès)
   ────────────────────────────────────────────────────────────
   ► Remplacer TP_AUTH_CLIENT_ID par votre Application (client) ID Azure AD.
   ► Dans le portail Azure → Entra ID → Inscriptions d'applications :
       Authentification → Ajouter une plateforme → Application monopage (SPA)
       URI de redirection : http://localhost:8000  (et l'URL de production)
   ════════════════════════════════════════════════════════════ */
const TP_AUTH_CLIENT_ID = 'VOTRE-CLIENT-ID-ICI'; // ← À remplacer

(function initMsalAuth() {
  /* Si MSAL n'est pas chargé (ex : offline) → accès direct sans auth */
  if (typeof msal === 'undefined') {
    document.getElementById('tpLoginOverlay')?.remove();
    return;
  }

  const _msalCfg = {
    auth: {
      clientId:              TP_AUTH_CLIENT_ID,
  ...
  const _loginScopes = ['openid', 'profile', 'email'];
  let   _msalInst    = null;
  let   _account     = null;

  function _showApp() {
    const ov = document.getElementById('tpLoginOverlay');
  ...
  function _updateChip() {
    const chip = document.getElementById('tpAuthChip');
    const name = document.getElementById('tpAuthName');
    if (!chip) return;
    if (_account) {
      chip.hidden = false;
      if (name) name.textContent = _account.name || _account.username || 'Connecté';
    } else {
      chip.hidden = true;
    }
  }
  function _showError(msg) {
    const el = document.getElementById('tpLoginError');
    if (!el) return;
    el.textContent = msg;
    el.hidden = false;
  }
  ...
  async function _doLogin() {
    if (!_msalInst) return;
    _clearError();
    const btn = document.getElementById('tpLoginBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'Connexion en cours…'; }
    try {
      const resp = await _msalInst.loginPopup({ scopes: _loginScopes });
      _account = resp.account;
      _showApp();
      _updateChip();
    } catch (e) {
      if (e.errorCode !== 'user_cancelled') {
        _showError('Erreur de connexion : ' + (e.errorMessage || e.message || 'inconnue'));
      }
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = ''; }
      /* Remet le contenu du bouton (l'image + texte ont été perdus par textContent) */
      _rebuildLoginBtn();
    }
  }

  function _rebuildLoginBtn() {
    const btn = document.getElementById('tpLoginBtn');
    if (!btn) return;
    btn.replaceChildren();
    const img = document.createElement('img');
    img.src = 'assets/MicrosoftN.png'; img.className = 'tp-login-ms-icon'; img.alt = '';
    btn.appendChild(img);
    btn.appendChild(document.createTextNode(' Se connecter avec Microsoft'));
  }

  function _doLogout() {
    if (!_msalInst || !_account) return;
    _msalInst.logoutPopup({ account: _account }).catch(() => {});
    _account = null;
    _updateChip();
    _showLogin();
  }
  ...
  /* Init */
  try {
    _msalInst = new msal.PublicClientApplication(_msalCfg);
    ...
    /* client_id non configuré ou MSAL indisponible → accès libre */
    console.warn('[TenantPulse Auth] MSAL init failed:', e.message);
    _showApp();
    return;
  }

  /* Câblage différé (le DOM est prêt car le script est defer) */
  window.addEventListener('load', function () {
    document.getElementById('tpLoginBtn')?.addEventListener('click', _doLogin);
    document.getElementById('tpAuthChip')?.addEventListener('click', _doLogout);
  });
})();
